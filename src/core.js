...
let isFun = obj => typeof obj === 'function' || obj instanceof Function;
let isPromise = obj => typeof obj === 'object' && isFun(obj.then);
let toPromise = (fun, args, ctx) => {
    if (isFun(fun)) {
...
let err = msg => { throw new Error(`[ISwitch Error] ${msg}`); }
let warn = msg => { console.warn(`[ISwitch Warn] ${msg}`); }
